...
class CompilerForQAOA:
    ...
    def send_quark(self, repeat, task_name: str = '', priority: int = 1):
        """
        Send the task to the quafu cloud platform.
        Args:
            repeat (int): shots = repeat * 1024.
            task_name (str): The name of the task so that you can query the task status
                             on the quafu cloud platform later.
            priority (int): Highest priority for users is 1
        Returns:
            task_id (str): The ID number of the task, which can uniquely identify the task.
        """
        openqasm, final_phys2logi_mapping, ScQ_circuit = self.graph_to_qasm()
        ...
        tmgr = quarktask(self._apitoken)
        task_id = tmgr.run(task, repeat=repeat)  # total shots = task['shots]*repeat
        print("The task has been submitted to the quafu cloud platform.\nThe task ID is '%s'" % task_id)
        print(tmgr.result(task_id))
        return task_id
    ...
    def quarktask_status_query(self, task_id: str):
        # ????????
        import sys
        sys.setrecursionlimit(3000)
        if self._current_recursion_depth > self._max_recursion_depth:
            print("Reached maximum recursion depth. Exiting recursion.")
            return None  # ??????????,????????

        self._current_recursion_depth += 1

        tmgr = quarktask(self._apitoken)
        task_status = tmgr.status(task_id)
        if task_status == "Finished":
            res = tmgr.result(task_id)
            self._current_recursion_depth = 0  # ?????????
            return res
        else:
            print("The current task status is '%s', please wait." % task_status)
            time.sleep(1)  # wait for 1 seconds before checking the status again
            result = self.quarktask_status_query(task_id)
            self._current_recursion_depth -= 1
            return result

    def quarktask_status_query2(self, task_id: str):
        tmgr = quarktask(self._apitoken)
        task_status = tmgr.status(task_id)
        if task_status == "Finished":
            res = tmgr.result(task_id)
            ...
            print("The current task status is '%s', please wait." % task_status)
            time.sleep(1)  # wait for 1 seconds before checking the status again
            return self.quarktask_status_query2(task_id)
    ...
    def left_right_counts_rearrange(self, logi2phys_mapping, counts):
        ...
        qubit_str = list(counts['count'].keys())
        counts_new = defaultdict(list)
        for i in range(len(qubit_str)):
            str = ''
            for k in range(len(logi2phys_mapping)):
                str = str + qubit_str[i][::][logi2phys_mapping[k]]
            counts_new[str] = counts['count'][qubit_str[i]]
    ...
    def quark_results_processing(self, counts):
        # The sampling result is the distribution of hardware physical qubit strings,
        # and the physical qubits need to be mapped back to the weight graph nodes.
        logi2phys_mapping = self._logi2phys_mapping
        counts_ScQ_new = self.left_right_counts_rearrange(logi2phys_mapping, counts)
        counts_ScQ = sorted(counts_ScQ_new.items(), key=lambda x: x[1], reverse=True)
        counts_ScQ = [(item[0], item[1]) for item in counts_ScQ]
